...
if calculator_category == "🌍 Atmospheric & Physiological":
    ...
    with spo2_tabs[0]:
        ...
        col1, col2 = st.columns(2)
        with col1:
            niermeyer_alt = st.number_input(
                "Altitude (m)", min_value=0, max_value=8848, value=4000, key="niermeyer_alt"
            )
        with col2:
        ...
        fig.update_layout(
            title="Niermeyer Model: SpO₂ vs Altitude",
    ...
    with spo2_tabs[1]:
        ...
        st.code(
            "SpO₂(t) = β₀ + β₁·SpO₂(t-12h) + β₂·SpO₂(t-24h) + β₃·HR(t-12h) + β₄·HR(t-24h) + β₅·Altitude + ε"
        )
    ...
    with spo2_tabs[3]:
        ...
        if st.button("Generate Comparison Plot", key="gen_comp"):
            ...
            fig = go.Figure()
            fig.add_trace(
                go.Scatter(x=alt_range, y=results_male, name=f"Niermeyer ({comp_sex})", mode="lines")
            )
            fig.add_trace(
                go.Scatter(
                    x=alt_range,
                    y=results_tushaus,
                    name="Tüshaus Cascade",
                    mode="lines",
                    line=dict(dash="dash"),
                )
            )
